...
class BudgetOptimizer:
  ...
  def optimize(
    ...
    Args:
      ...
      end_date: Optional end date selector, *inclusive* in _yyyy-mm-dd_ format.
        Both start and end time selectors should align with the Meridian time
        dimension coordinates in the underlying model if optimizing the original
        data. If `new_data` is provided with a different number of time periods
        than in `InputData`, then the start and end time coordinates must match
        the time dimensions in `new_data.time`. By default, all times periods
        are used. Either start or end time component can be `None` to represent
        the first or the last time coordinate, respectively.
  ...
  def _validate_grid(
    ...
    if not np.array_equal(selected_times, optimization_grid.selected_times):
      warnings.warn(
          'Given optimization grid was created with `selected_times` ='
          f' {optimization_grid.selected_times}, but optimization was called'
          f' with `selected_times` = {selected_times}. A new grid will be'
          ' created.'
      )
      return False
