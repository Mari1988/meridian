"""Ablation: does near-zero DGP noise let the informed variant hit ~0% ROI error?

Follow-up to `scratch_ablation_exact_ec.py` (exact ec_m barely moved roi_m's
bias -- ruling out the ec_m<->roi_m coupling as the main driver) and
`scratch_check_roi_posterior_stat.py` (posterior mean ~= median -- ruling out
a reporting-statistic artifact). By elimination, the remaining candidate is
the DGP's own noise: `realistic_baseline.py`'s persistent, cross-geo-
correlated AR(1) shock plus geo-idiosyncratic residual, calibrated to an
oracle R^2 of 0.80 (`RealisticBaselineConfig`, `calibrate_to_oracle_r2`).

This tests that directly: push `target_oracle_r2` from 0.80 toward 0.99 (near
-noiseless -- the noise multiplier that scales both the common AR(1) shock
and the geo-idiosyncratic residual, per `RealisticBaselineSimulator.
apply_noise_scale`, shrinks toward ~0) while leaving `target_roi` at the
original, non-confounded 8.0/5.0 setting (scratch_ablation_low_roi.py showed
lowering target_roi also lowers media_share_pct, a confound this script
avoids by not touching it). Fits both 'default' and 'ec_alpha_only' (exact
ec_m + informed alpha_m) per seed, on a SMALL seed set (2-3) since this is a
targeted mechanism check, not a full sweep.

Prediction: if DGP noise is really the driver, 'ec_alpha_only' roi_m error
should collapse toward 0% here, while 'default' should stay heavily biased
regardless (its failure mode is ec_m prior-dominance, not noise -- it should
still show ec_m error close to what it always shows, ~-60 to -85%,
independent of the noise level).

Usage:
  # one seed (oracle_r2 defaults to 0.99):
  .venv/bin/python demo/synthetic/scratch_ablation_low_noise.py <seed> <out_dir> [oracle_r2]

  # 2-3 seeds, from repo root:
  OUT=demo/synthetic/fitted_models/scratch_ablation_low_noise
  mkdir -p "$OUT"
  for seed in 1320 7 42; do
    .venv/bin/python demo/synthetic/scratch_ablation_low_noise.py "$seed" "$OUT"
  done

  # then assemble + report (joins the original oracle_r2=0.80 ablation run for
  # a before/after comparison):
  .venv/bin/python demo/synthetic/scratch_ablation_low_noise.py --report "$OUT" \
      demo/synthetic/fitted_models/scratch_ablation_exact_ec

Not part of the tracked pipeline -- a standalone scratch script, self-contained.
"""
from __future__ import annotations

import glob
import os
import sys
import time
import warnings

warnings.filterwarnings('ignore')
os.environ.setdefault('TF_CPP_MIN_LOG_LEVEL', '2')

import numpy as np
import pandas as pd

HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, HERE)

from meridian.model import model

from data_simulator import SimulationConfig
from model_utils import build_model_spec
import realistic_baseline as rb

MCMC_KWARGS = dict(n_chains=2, n_adapt=500, n_burnin=500, n_keep=1000, seed=1)
N_KNOTS = 8
TRUE_EC_TV = 9.0
TRUE_ALPHA_TV = 0.3
DEFAULT_ORACLE_R2 = 0.99

BASE_OVERRIDES = {
    'n_imp_channels': 2,
    'channel_names': ['TV', 'Display'],
    'target_audience_pop_frac': {'TV': 0.60, 'Display': 0.50},
    'current_reach_frac': {'TV': 0.10, 'Display': 0.5},
    'frequency_range': {'TV': (1, 2), 'Display': (1, 5)},
    'target_roi': {'TV': 8.0, 'Display': 5.0},  # original, non-confounded setting
    'max_lag': 13,
    'n_times': 156,
    'roi_ec_elasticity': -0.3,
    'roi_alpha_elasticity': 0.3,
    'adstock_retention_range': {'TV': (TRUE_ALPHA_TV, TRUE_ALPHA_TV), 'Display': (0.0, 0.3)},
    'baseline_scale': 0.065,
}


def _real_df(study_dir: str) -> pd.DataFrame:
  cache = os.path.join(study_dir, '.cache_geo_media_rf.csv')
  if not os.path.exists(cache):
    pd.read_csv(
        'https://raw.githubusercontent.com/google/meridian/refs/heads/main/'
        'meridian/data/simulated_data/csv/geo_media_rf.csv'
    ).to_csv(cache, index=False)
  return pd.read_csv(cache)


def run_seed(seed: int, target_r2: float, study_dir: str) -> dict:
  real_df = _real_df(study_dir)

  def build(saturation_frequency_tv=None, r2=target_r2):
    overrides = dict(BASE_OVERRIDES, seed_num=seed)
    if saturation_frequency_tv is not None:
      overrides['saturation_frequency'] = {
          'TV': saturation_frequency_tv, 'Display': 4.0}
    cfg = SimulationConfig.from_dict(overrides)
    sim, data, gt = rb.build_real_augmented_realistic(
        cfg, real_df,
        rf_source_map={'TV': 'Channel3'},
        plain_source_map={'Display': 'Channel2'},
        target_oracle_r2=r2, n_knots=N_KNOTS,
    )
    return cfg, sim, data, gt

  _, sim_base, _, _ = build(r2=None)
  ec_base = float(sim_base.ec_m.numpy()[0])
  sat_freq = 4.0 * TRUE_EC_TV / ec_base

  cfg, sim, data, gt = build(sat_freq)
  assert abs(float(sim.ec_m.numpy()[0]) - TRUE_EC_TV) < 0.05, (
      f'seed {seed}: ec_m solve missed ({float(sim.ec_m.numpy()[0])})')
  assert abs(float(sim.alpha_m.numpy()[0]) - TRUE_ALPHA_TV) < 1e-6, (
      f'seed {seed}: TV alpha wrong')

  diag = rb.baseline_diagnostics(sim, N_KNOTS)
  true_roi = float(np.asarray(gt['roi_m'])[0])
  row = {
      'seed': seed,
      'target_oracle_r2': target_r2,
      'achieved_oracle_r2': round(diag['oracle_r2'], 4),
      'noise_scale': round(diag['noise_scale'], 5),
      'media_share_pct': round(diag['media_share_pct'], 1),
      'true_roi_m': round(true_roi, 3),
  }

  def fit(variant):
    spec = build_model_spec(variant, sim, cfg, media_prior_type='roi',
                             knots=N_KNOTS, max_lag=None)
    mmm = model.Meridian(input_data=data, model_spec=spec)
    mmm.sample_prior(500)
    mmm.sample_posterior(**MCMC_KWARGS)
    return mmm

  mmm = fit('default')
  post = mmm.inference_data.posterior
  row['default_ec_err'] = round((float(post['ec_m'].values[..., 0].mean()) / TRUE_EC_TV - 1) * 100, 1)
  row['default_roi_err'] = round((float(post['roi_m'].values[..., 0].mean()) / true_roi - 1) * 100, 1)
  row['default_alpha_err'] = round((float(post['alpha_m'].values[..., 0].mean()) / TRUE_ALPHA_TV - 1) * 100, 1)
  del mmm

  mmm = fit('ec_alpha_only')  # exact ec_m + informed alpha_m
  post = mmm.inference_data.posterior
  row['exact_ec_err'] = round((float(post['ec_m'].values[..., 0].mean()) / TRUE_EC_TV - 1) * 100, 1)
  row['exact_roi_err'] = round((float(post['roi_m'].values[..., 0].mean()) / true_roi - 1) * 100, 1)
  row['exact_alpha_err'] = round((float(post['alpha_m'].values[..., 0].mean()) / TRUE_ALPHA_TV - 1) * 100, 1)
  del mmm

  return row


def main_report(out_dir: str, hi_noise_ablation_dir: str | None) -> None:
  files = sorted(glob.glob(os.path.join(out_dir, 'per_seed_*.csv')))
  if not files:
    print(f'no per_seed_*.csv files found in {out_dir}')
    return
  df = (pd.concat([pd.read_csv(f) for f in files], ignore_index=True)
        .sort_values('seed').reset_index(drop=True))
  df.to_csv(os.path.join(out_dir, 'per_seed_all.csv'), index=False)

  pd.set_option('display.width', 200)
  print(f'{len(df)} seeds completed -- target_oracle_r2={df["target_oracle_r2"].iloc[0]}\n')
  print(df[['seed', 'achieved_oracle_r2', 'noise_scale', 'media_share_pct',
            'default_ec_err', 'default_roi_err', 'exact_ec_err', 'exact_roi_err']
           ].to_string(index=False))
  print()

  if hi_noise_ablation_dir:
    hi_path = os.path.join(hi_noise_ablation_dir, 'per_seed_all.csv')
    if os.path.exists(hi_path):
      hi = pd.read_csv(hi_path)[['seed', 'exact_ec_err', 'exact_roi_err']].rename(
          columns={'exact_ec_err': 'hi_noise_exact_ec_err',
                   'exact_roi_err': 'hi_noise_exact_roi_err'})
      merged = df.merge(hi, on='seed', how='inner')
      if len(merged):
        print('--- low-noise (this run) vs original oracle_r2=0.80 ablation ---')
        print(merged[['seed', 'exact_roi_err', 'hi_noise_exact_roi_err']].to_string(index=False))
        print()
        print(f"low-noise  exact-ec roi_err: medianAbs={merged['exact_roi_err'].abs().median():.1f}  median={merged['exact_roi_err'].median():+.1f}")
        print(f"orig-noise exact-ec roi_err: medianAbs={merged['hi_noise_exact_roi_err'].abs().median():.1f}  median={merged['hi_noise_exact_roi_err'].median():+.1f}")


def main() -> int:
  if sys.argv[1:2] == ['--report']:
    out_dir = sys.argv[2]
    hi_noise_ablation_dir = sys.argv[3] if len(sys.argv) > 3 else None
    main_report(out_dir, hi_noise_ablation_dir)
    return 0

  seed = int(sys.argv[1])
  out_dir = sys.argv[2]
  target_r2 = float(sys.argv[3]) if len(sys.argv) > 3 else DEFAULT_ORACLE_R2
  study_dir = HERE
  os.makedirs(out_dir, exist_ok=True)
  out_path = os.path.join(out_dir, f'per_seed_{seed}.csv')

  if os.path.exists(out_path):
    print(f'seed {seed}: already done, skipping', flush=True)
    return 0

  t0 = time.time()
  row = run_seed(seed, target_r2, study_dir)
  pd.DataFrame([row]).to_csv(out_path, index=False)
  print(
      f"seed {seed} done in {time.time() - t0:.0f}s | achieved_r2={row['achieved_oracle_r2']:.4f} "
      f"noise_scale={row['noise_scale']:.4f} | "
      f"default ec {row['default_ec_err']:+.1f}% roi {row['default_roi_err']:+.1f}% | "
      f"exact-ec ec {row['exact_ec_err']:+.1f}% roi {row['exact_roi_err']:+.1f}%",
      flush=True)
  return 0


if __name__ == '__main__':
  sys.exit(main())
