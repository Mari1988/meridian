"""Slide 7 candidate: recovery box plot (like slide 10's style) at r2=0.99.
...
line) -- but using this session's settled basis instead: alpha_m=0.3,
oracle_r2 target 0.99 (achieved ~0.984-0.986), 3 seeds (1320, 7, 42), from
`scratch_ablation_low_noise.py`'s per_seed_all.csv.

TV only (that data doesn't have a Display channel comparison the way the
canonical run's table does).

Usage:
  .venv/bin/python demo/synthetic/scratch_plot_recovery_boxplot_r99.py
"""
...
OUT_PATH = os.path.join(HERE, 'figures', 'slide7_recovery_boxplot_r99.png')
...
def main() -> None:
  df = pd.read_csv(os.path.join(
      HERE, 'fitted_models', 'scratch_ablation_low_noise', 'per_seed_all.csv'
  )).sort_values('seed')
  ...
  fig.suptitle(
      f'TV channel, {len(df)} seeds, oracle_r2~=0.99, alpha_m=0.3 -- '
      'saturation point recovers cleanly; ROI and adstock do not fully',
