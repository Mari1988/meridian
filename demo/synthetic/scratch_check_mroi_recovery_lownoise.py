"""Does the mROI-at-elevated-spend story hold at the near-noiseless setting?

Low-noise counterpart to `scratch_check_mroi_recovery.py`, which confirmed
the mROI-at-elevated-spend discriminator at the STANDARD realistic setting
(oracle_r2=0.80): default's mROI error compounds catastrophically with spend
multiplier (median +7.7% at 1x to -36.2% at 5x) while the exact-ec_m informed
variant stays flat (~14-15% across every multiplier). This is the basis this
session has settled on for the deck (alpha_m=0.3, oracle_r2~=0.99, not 0.80),
so the mROI check needs re-running at that setting before quoting it -- the
0.80 numbers above are NOT assumed to carry over.

Same 3 seeds as `scratch_ablation_low_noise.py` (1320, 7, 42), same
oracle_r2=0.99 target (achieved ~0.984-0.986 -- the noise multiplier hits its
search floor before reaching exactly 0.99, per that script's own finding),
target_roi TV=8.0/Display=5.0, alpha_m=0.3. At that setting, exact-ec_m
roi_m-at-1x errors were: seed 1320 +9.9%, seed 7 +7.5%, seed 42 +8.1% -- the
reference point for "does mROI look better than plain roi_m here too."
